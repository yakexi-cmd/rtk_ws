#include "dlio/odom.h"
ros::Time start_time,start_time0;
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  start_time0 = ros::Time::now();
  this->odom_pub     = this->nh.advertise<nav_msgs::Odometry>("/Odometry", 1, true);
  // this->odom_pub     = this->nh.advertise<nav_msgs::Odometry>("odom", 1, true);
  this->imu_odom_pub     = this->nh.advertise<sensor_msgs::PointCloud2>("/imu_odom", 1, true);
  ...
  this->deskewed_pub = this->nh.advertise<sensor_msgs::PointCloud2>("/velodyne_cloud_registered", 1, true);
  ...
  this->concave_hull.setAlpha(this->keyframe_thresh_dist_);//设置凹包计算中的alaph值，通常和关键帧的距离阈值相关联，定义每个点到最近邻点的最大距离
  this->concave_hull.setKeepInformation(true);//凹包算法保留每个点的原始索引信息
  ...
  //TODO:配置裁剪器，会将超出范围的点云去除，范围由crop_size_指定
  ...
  fclose(file);
  // ros::Duration duration_time0 = ros::Time::now() - start_time0 ;
  // std::cout<<"=======duration_time0(ns)========="<<duration_time0.toSec()<<std::endl;
...
void dlio::OdomNode::getParams() {
  ...
  // Deskew FLag
  ros::param::param<bool>("~dlio/pointcloud/deskew", this->deskew_, false);

  // Gravity
  ros::param::param<double>("~dlio/odom/gravity", this->gravity_, 9.80665);

  // Compute time offset between lidar and imu
  ros::param::param<bool>("~dlio/odom/computeTimeOffset", this->time_offset_, true);
  ...
  this->extrinsics.baselink2imu.R =
    Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(baselink2imu_R.data(), 3, 3);//Eigen::Map直接将vector<float>映射成matrix
  ...
  // scale-misalignment matrix 表示imu尺度误差校准矩阵
  ...
  //imu需要校准，刚开始时初始化为0，之后通过标定得到bias和scale
  ...
  // Geometric Observer：几何观测器，用于估计和校准
  ros::param::param<double>("~dlio/odom/geo/Kp", this->geo_Kp_, 1.0);//位置误差的比例增益控制器参数
  ros::param::param<double>("~dlio/odom/geo/Kv", this->geo_Kv_, 1.0);//速度误差的
  ros::param::param<double>("~dlio/odom/geo/Kq", this->geo_Kq_, 1.0);//姿态（四元数或旋转矩阵的）比例增益控制器参数
  ros::param::param<double>("~dlio/odom/geo/Kab", this->geo_Kab_, 1.0);//加速度计偏差的控制增益
  ros::param::param<double>("~dlio/odom/geo/Kgb", this->geo_Kgb_, 1.0);//陀螺仪偏差的控制增益
  ros::param::param<double>("~dlio/odom/geo/abias_max", this->geo_abias_max_, 1.0);//加速度偏差的最大约束，确保陀螺仪偏差不会超过合理范围，避免过大的校正或发散
...
void dlio::OdomNode::publishPose(const ros::TimerEvent& e) {
  ...
  this->odom_ros.header.stamp = ros::Time::now();
  // start_time = ros::Time::now();
  //odom是世界坐标系，在launch文件中被映射为map坐标系，baselink_frame坐标系是机器人本体的坐标系，用于和lidar，imu进行坐标变换
  ...
  // ros::Duration duration_time0 = ros::Time::now() - start_time ;
  // std::cout<<"=======duration_time0(ns)========="<<duration_time0<<std::endl;
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  ...
  br.sendTransform(transformStamped);
  // ros::Duration publishToROS_duration = ros::Time::now() - publishToROS_time;
  // std::cout<<"======publishToROS_duration======"<<publishToROS_duration<<std::endl;

}
ros::Time publishCloud_time;
void dlio::OdomNode::publishCloud(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  publishCloud_time = ros::Time::now();
  ...
  pcl::transformPointCloud (*published_cloud, *deskewed_scan_t_, T_cloud); //将点云通过变换矩阵T_cloud变换到新的坐标系下，该代码的作用是点云去畸变，将去除畸变的点云存入deskewed_scan_t_

  // published deskewed cloud
  sensor_msgs::PointCloud2 deskewed_ros;
  pcl::toROSMsg(*deskewed_scan_t_, deskewed_ros);//TODO:1210 change
  // pcl::toROSMsg(*published_cloud, deskewed_ros);
  ...
  // ros::Duration publishCloud_duration = ros::Time::now() - publishCloud_time;
  // std::cout<<"======publishCloud_duration======"<<publishCloud_duration<<std::endl;

}
ros::Time publishKeyframe_time;
void dlio::OdomNode::publishKeyframe(std::pair<std::pair<Eigen::Vector3f, Eigen::Quaternionf>, pcl::PointCloud<PointType>::ConstPtr> kf, ros::Time timestamp) {
  publishKeyframe_time = ros::Time::now();
  // std::cout<<"===========publishKeyframe_time============"<<std::endl;
  // Push back
  ...
  // ros::Duration publishKeyframe_duration = ros::Time::now() - publishKeyframe_time;
  // std::cout<<"======publishKeyframe_duration======"<<publishKeyframe_duration<<std::endl;
}
ros::Time getScanFromROS_time;
void dlio::OdomNode::getScanFromROS(const sensor_msgs::PointCloud2ConstPtr& pc) {
  getScanFromROS_time = ros::Time::now();
  pcl::PointCloud<PointType>::Ptr original_scan_ (boost::make_shared<pcl::PointCloud<PointType>>());
  pcl::fromROSMsg(*pc, *original_scan_); //把pc转换为pcl格式
  ...
  // ros::Duration getScanFromROS_duration = ros::Time::now() - getScanFromROS_time;
  // std::cout<<"======getScanFromROS_duration======"<<getScanFromROS_duration<<std::endl;
}
ros::Time preprocessPoints_time;
void dlio::OdomNode::preprocessPoints() {
  preprocessPoints_time = ros::Time::now();
  // Deskew the original dlio-type scan
  if (this->deskew_) {
    // std::cout<<"======deskew_========="<<std::endl;
    ...
    if (!this->first_valid_scan) {
      ...
      // IMU prior for second scan onwards ：如果先得到了imu数据，使用imu积分得到先验位姿
  ...
  // ros::Duration preprocessPoints_duration = ros::Time::now() - preprocessPoints_time;
  // std::cout<<"======preprocessPoints_duration======"<<preprocessPoints_duration<<std::endl;

}
ros::Time deskewPointcloud_time;
void dlio::OdomNode::deskewPointcloud() {
  deskewPointcloud_time = ros::Time::now();
  pcl::PointCloud<PointType>::Ptr deskewed_scan_ (boost::make_shared<pcl::PointCloud<PointType>>());
  deskewed_scan_->points.resize(this->original_scan->points.size());

  // individual point timestamps should be relative to this time
  double sweep_ref_time = this->scan_header_stamp.toSec();
  //按照时间戳对点进行排序并构建时间戳列表：具体实现是通过定义比较函数，point_time_cmp和不等于point_time_neq
  // sort points by timestamp and build list of timestamps
  std::function<bool(const PointType&, const PointType&)> point_time_cmp;//比较PointType类型的点的时间戳
  std::function<bool(boost::range::index_value<PointType&, long>,//boost::range::index_value通常用在去重操作中
                     boost::range::index_value<PointType&, long>)> point_time_neq;
  std::function<double(boost::range::index_value<PointType&, long>)> extract_point_time; //用于从点中提取时间戳
  ...
  // filter unique timestamps 去除具有相同时间戳的点
  ...
  if (this->time_offset_) {
    offset = sweep_ref_time - extract_point_time(*points_unique_timestamps.begin());//计算扫描参考时间和第一个点的时间戳的偏移量
    // std::cout<<"=====时间偏移量为："<<offset<<"======="<<std::endl;
    
  }
  
  // build list of unique timestamps and indices of first point with each timestamp 遍历去重后的点，提取唯一时间戳并计算相对于扫描参考时间的偏移量，同时存储每个时间戳对应的点的索引，unique_time_indices放入这里
  ...
  //TODO:我理解的是通过imu预积分，如果得到的帧数和雷达的帧数一样，就代表imu和雷达数据在时间上是同步的，之后对每一帧雷达点云使用imu积分得到的变换矩阵，校正激光雷达点云由于雷达传感器移动造成的位置偏差，经过校准后的点云是相对于雷达坐标系的，需要转换到世界坐标系下
...
#pragma omp parallel for num_threads(this->num_threads_)
  ...
  this->deskewed_scan = deskewed_scan_;
  this->deskew_status = true;
  // ros::Duration deskewPointcloud_duration = ros::Time::now() - deskewPointcloud_time;
  // std::cout<<"======deskewPointcloud_duration======"<<deskewPointcloud_duration<<std::endl;
}

ros::Time initializeInputTarget_time;
void dlio::OdomNode::initializeInputTarget() {
  initializeInputTarget_time = ros::Time::now();
  ...
  // ros::Duration initializeInputTarget_duration = ros::Time::now() - initializeInputTarget_time;
  // std::cout<<"======deskewPointcloud_duration======"<<initializeInputTarget_duration<<std::endl;
}
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  // std::cout<<"========发布imu_odom_pub=========="<<std::endl;
  ...
  for(const auto &point : this->original_scan->points)
  {
    ...
    pcl_point.y = point.y;
    pcl_point.z = point.z;
    mid_cloud->points.push_back(pcl_point);
  }  

  pcl::toROSMsg(*mid_cloud,origin_cloud);
  origin_cloud.header.frame_id ="map";
  origin_cloud.header.stamp = this->scan_header_stamp;
  imu_odom_pub.publish(origin_cloud);
  ...
  this->setInputSource();
  // std::cout<<"===================keyframes.size====================="<<this->keyframes.size()<<std::endl;
  ...
  // //在这里单独发布imu_odom
  // sensor_msgs::PointCloud2 imu_odom_ros;
  // pcl::toROSMsg(*published_cloud, imu_odom_ros);//TODO:1210 change
  // // pcl::toROSMsg(*published_cloud, deskewed_ros);
  // imu_odom_ros.header.stamp = this->scan_header_stamp;
  // imu_odom_ros.header.frame_id = this->odom_frame;
  // imu_odom_pub.publish(imu_odom_ros); //在这里增加了发布published_cloud，看一下是哪里产生的延迟
  ...
  // ros::Duration callbackPointCloud_duration = ros::Time::now() - callbackPointCloud_time;
  // std::cout<<"===========callbackPointCloud_duration========="<<callbackPointCloud_duration<<std::endl;

}
ros::Time callbackImu_time;
void dlio::OdomNode::callbackImu(const sensor_msgs::Imu::ConstPtr& imu_raw) {
  callbackImu_time = ros::Time::now();
  ...
  // ros::Duration callbackImu_duration = ros::Time::now() - callbackImu_time;
  // std::cout<<"===========callbackImu_duration========="<<callbackImu_duration<<std::endl;
}
ros::Time getNextPose_time;
void dlio::OdomNode::getNextPose() {
  getNextPose_time = ros::Time::now();
  ...
  if (this->new_submap_is_ready && this->submap_hasChanged) {
    ...
    // ros::Duration getNextPose_duration = ros::Time::now() - getNextPose_time;
    // std::cout<<"===========getNextPose_duration========="<<getNextPose_duration<<std::endl;
  }
