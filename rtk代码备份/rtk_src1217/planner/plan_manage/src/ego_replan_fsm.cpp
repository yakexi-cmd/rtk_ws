...
void EGOReplanFSM::multi_goal_callback(const geometry_msgs::PoseArray::ConstPtr &msg)
{
  ...
  multi_goal_finish =0;
  // multi_goal_finish=false;
  for(const auto &points:msg->poses)
  {
    pts << points.position.x , points.position.y , odom_pos_(2);
    multi_goal.push(pts);
  }
  // ros::Rate rate(10);  // 设定回调的频率为 10Hz
  // end_pt_ = multi_goal.front(); //选择最早加入的目标点,将终点目标设置为当前路径中的第一个路标点
  // multi_goal.pop();
  while(!multi_goal.empty()){
    std::cout<<"***************************multi_goal.size()**********************************"<<multi_goal.size()<<std::endl;
    std::cout<<"************************************trigger_**********************************"<<trigger_<<std::endl;
    std::cout<<"===========multi_goal_finish============="<<multi_goal_finish<<std::endl;
    //如果当前点在执行，则等同于执行单点的路径规划方法，按照goal_callback继续执行即可
    end_pt_ = multi_goal.front(); //选择最早加入的目标点,将终点目标设置为当前路径中的第一个路标点
    trigger_ = true;
    init_pt_ = odom_pos_;
    bool success = false;
    std::cout<<"-----------"<<end_pt_(0)<<","<<end_pt_(1)<<std::endl;
    success = planner_manager_->planGlobalTraj(odom_pos_,odom_vel_,Eigen::Vector3d::Zero(),end_pt_,Eigen::Vector3d::Zero(),Eigen::Vector3d::Zero());
    visualization_->displayGoalPoint(end_pt_,Eigen::Vector4d(0,0.5,0.5,1),0.3,0);
...
//      std::cout << "test t_cur and t_duration_  : " << t_cur << "  " <<  info->duration_ << "\n";
      ...
      if(have_multi_)
      {
        if((end_pt_ - pos).norm() < 0.5)
        {
          ...
          changeFSMExecState(WAIT_TARGET, "FSM");
          std::cout<<"235235##############到达目标点！！！###############"<<std::endl;
        }else{
            if(dis_closest > 0.2)
            {
                // changeFSMExecState(REPLAN_TRAJ, "FSM");
