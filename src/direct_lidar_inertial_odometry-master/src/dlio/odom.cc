...
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  this->publish_timer = this->nh.createTimer(ros::Duration(0.025), &dlio::OdomNode::publishPose, this);

  // this->cloudPublish_timer = this->nh.createTimer(ros::Duration(0.01), &dlio::OdomNode::publishCloud, this);
...
void dlio::OdomNode::publishPose(const ros::TimerEvent& e) {
  ...
  // ros::Duration deltat;
  // deltat = ros::Time::now() - last_time;
  // std::cout<<"===============get pose============="<<deltat.toSec()<<"s"<<std::endl;
  // last_time=ros::Time::now();

}

// void dlio::OdomNode::publishCloud(const ros::TimerEvent& e)
// {
    // return;
    // Publish stuff to ROS
    // if(this->current_scan->points.size() > 0 || this->deskewed_scan->points.size() > 0)
    // {
...
void dlio::OdomNode::publishCloud(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  ...
  // pcl::PointCloud<PointType>::Ptr deskewed_scan_t_filtered_ (boost::make_shared<pcl::PointCloud<PointType>>());
  // // radius_
  // pcl::RadiusOutlierRemoval<PointType> outrem;
  // outrem.setInputCloud(deskewed_scan_t_);
  // outrem.setRadiusSearch(0.1); // 设置半径搜索范围  
  // outrem.setMinNeighborsInRadius(3); // 设置最小邻居数目
  // outrem.filter(*deskewed_scan_t_filtered_);
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr& pc) {
  ...
  ros::Duration deltat;
  deltat = ros::Time::now() - start_time_getNextPose;
  std::cout<<"*************callbackPointCloud**********************"<<deltat.toSec()<<"s"<<std::endl;
