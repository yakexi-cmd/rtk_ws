...
#include <queue>
#include <ros/ros.h>
#include <tuple>//tuple是c++中的标准库类模板，用于存储固定数量的异类对象，eg:std::tuple
...
struct matrix_hash:std::unary_function<T,size_t>{
    std::size_t operator()(T const& matrix) const{
        ...
        for(size_t i=0;i<matrix.size();++i){
            ...
            seed ^=std::hash<typename T:Scalar>()(elem) + + 0x9e3779b9 + (seed <<6 )+(seed >>2) ;//typename T:Scalar通常表示矩阵的标量类型
        }
        return seed;
    }
};
...
struct MappingParameters {
    ...
    double filter_height_min_,filter_height_max_; // 去除地面高度
    ...
    /* raycasting */
    double p_hit_, p_miss_, p_min_, p_max_, p_occ_;  // occupancy probability 光线与物体相交、未相交的概率
    double prob_hit_log_, prob_miss_log_, clamp_min_log_, clamp_max_log_,
        min_occupancy_log_;                   // 存储占用概率的对数值，在概率模型中使用对数形式进行数值稳定性处理。或用于计算对数几率logit of occupancy probability
    ...
    bool mode_shift_ = false;//系统不同模式间的切换
...
private:
    ...
    void modeShiftCallback(const std_msgs::BoolConstPtr& msg);

    // update occupancy by raycasting 过滤本地地图，更新占用信息，可视化回调函数
    void fliterLocalMap();
    void updateOccupancyCallback(const ros::TimerEvent& /*event*/);
    void visCallback(const ros::TimerEvent& /*event*/);
    // main update process
    void projectDepthImage();
    void raycastProcess();//射线投射处理
    void clearAndInflateLocalMap();//清除和膨胀本地地图
    void projectMapTo2d();//将地图投影到2d
    //TODO:膨胀点云
    ...
    //extra function
    void showFrontPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_all);
    ...
    shared_ptr<message_filters::Subscriber<sensor_msgs::Image>> depth_sub_;//将话题订阅与同步器结合使用，同时depth_sub_这些是订阅器的智能指针
    shared_ptr<message_filters::Subscriber<geometry_msgs::PoseStamped>> pose_sub_;
    shared_ptr<message_filters::Subscriber<nav_msgs::Odometry>> odom_sub_;
    SynchronizerImagePose sync_image_pose_;
    SynchronizerImageOdom sync_image_odom_;

    ros::Subscriber indep_cloud_sub_, indep_odom_sub_,indep_cloud_segmentation_sub_, mode_shift_sub_, local_cloud_sub_;
    ros::Publisher map_pub_, map_inf_pub_,map_2d_pub_, map_2d_pub_rviz_, terrain_height_pub_,map2d_pub_;
    ros::Publisher unknown_pub_, front_pub_;
...
inline void GridMap::boundIndex(Eigen::Vector3i& id) {
    Eigen::Vector3i id1;
    id1(0) = max(min(id(0),mp_map_vocel_num_(0)-1),0);
    id1(1) = max(min(id(1),mp_map_vocel_num_(1)-1),0);
    id1(2) = max(min(id(2),mp_map_vocel_num_(2)-1),0);
}
...
inline int GridMap::(Eigen::Vector2d pos) {
    if(!isInMap2d) {
        ROS_INFO("Run here!");
        return -1;
    }
    Eigen::Vector2i id;
    posToIndex2d(pos,id);
    return int(md_.occupancy_buffer_inflate_2d[toAddress2d(id(0),id(1))]);
}
...
inline bool GridMap::isInMap2d(const Eigen::Vector2d &pos){
    if(pos(0)<mp.map_min_boundary_(0)+1e-4 || pos(1) < mp.map_min_boundary_(1)+1e-4) return false;
    if(pos(1)>mp.map_max_boundary_(0)-1e-4 || pos(1) >mp.map_max_boundary_(1) - 1e-4) return false;
    return true;
}
...
inline void GridMap::posToIndex2d(const Eigen::Vector2d& pos,Eigen::Vector2i& id){
    for(int i=0;i<2;++i) id(i)=floor((pos(i)-mp.map_origin_(i))*mp.resolution_inv_);
}

inline void GridMap::indexToPos(const Eigen::Vector3i& id, Eigen::Vector3d& pos) {
  for (int i = 0; i < 3; ++i) pos(i) = (id(i) + 0.5) * mp_.resolution_ + mp_.map_origin_(i);//+0.5是为了确保体素转换到实际位置时，得到的是体素的中心位置，而不是体素的边界
}
