...
namespace ego_planner
{
    ...
    {
        nh.param("optimization/lambda_smooth", lamb da1_, -1.0);
    ...
    std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
    {
        ...
        // 确定步长，用于在轨迹点之间插值生成采样点
        double step_size = grid_map_->getResolution() / ((init_points.col(0) - init_points.rightCols(1)).norm() / (init_points.cols() - 1)) / 2;//TODO:没看懂 计算平均间距
        int in_id, out_id;
        ...
        for (int i = order_; i <= i_end; ++i) //遍历轨迹中的点
        {
            for (double a = 1.0; a >= 0.0; a -= step_size) //利用插值的方法在轨迹点钟生成采样点
            {
                ...
                {
                    ++same_occ_state_times; //用于跟踪当前状态持续的次数
                }
    ...
    {
        cost = 0.0;
        int end_idx = q.cols() - order_;//计算轨迹的末尾索引
        double demarcation = cps_.clearance; //路径的安全距离
        ...
        {
            check_collision_and_rebound(); //检测路径和障碍物的碰撞，并进行路径反弹
        }
    ...
    //计算b样条优化问题的适应度函数 输入：q控制点，输出：代价值和梯度矩阵
    ...
    //检测是否发生碰撞并执行反弹
    void BsplineOptimizer::check_collision_and_rebound(void)
    {
        Eigen::Vector3d temp_pos3d;
        Eigen::Vector2d temp_pos2d;
        ...
        for (int i = order_ - 1; i <= i_end; ++i)
        {
            ...
            /*** check if the new collision will be valid 检测碰撞的有效性***/
            if (occ)
            {
                for (size_t k = 0; k < cps_.direction[i].size(); ++k)
                {
                    cout.precision(2);
                    //对每个点的方向向量，计算当前点和基础点之间的点积，如果点积小于网格分辨率，则认为该点没有发生实际碰撞
            ...
            if (occ)
            {
                flag_new_obs_valid = true; //表示有障碍物

                int j;
                for (j = i - 1; j >= 0; --j) //判断起始点有没有在障碍物里
                {
                ...
                for (j = i + 1; j < cps_.size; ++j) //判断终点有没有在障碍物内
                {
                    temp_pos3d = cps_.points.col(j);
                    temp_pos2d<<temp_pos3d(0),temp_pos3d(1);
                    occ = grid_map_->getInflateOccupancy2d(temp_pos2d);
        ...
        if (flag_new_obs_valid) //如果检测到新的障碍物，进入路径规划和控制点的更新
        {
            vector<vector<Eigen::Vector3d>> a_star_pathes;
            for (size_t i = 0; i < segment_ids.size(); ++i)
            {
                /*** a star search ***/
                //探索的起点是segment_ids[i].first，终点是segment_ids[i].second
            ...
            for (size_t i = 0; i < segment_ids.size(); ++i)
            {
                ...
                // step 2
                int got_intersection_id = -1;
                //遍历路径中的每个控制点，计算控制点和A*的交点
                for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
                {
                    Eigen::Vector3d ctrl_pts_law(cps_.points.col(j + 1) - cps_.points.col(j - 1)), intersection_point;
                    int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
                    double val = (a_star_pathes[i][Astar_id] - cps_.points.col(j)).dot(ctrl_pts_law), last_val = val;
                    //TODO:计算控制点和A*路径中某点的点积val，若点积>0,表示路径上的点在当前控制点的前方
                    while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
                    {
                        last_Astar_id = Astar_id;
                        ...
                        if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) // val = last_val = 0.0 is not allowed
                        {
                            ...
                            got_intersection_id = j; //如果找到交点，标记当前控制点
                            break;
                        }
                    }
                    if (got_intersection_id >= 0)
                    {
                        ...
                        if (length > 1e-5)
                        {
                            //在控制点和交点之间进行插值，并检查每个点是否和障碍物发生碰撞
                            for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
                            {
                                temp_pos3d = (a / length) * intersection_point + (1 - a / length) * cps_.points.col(j);
                                temp_pos2d<<temp_pos3d(0),temp_pos3d(1);
                                ...
                                if (occ || a < grid_map_->getResolution())//如果当前被障碍物占据，或当前距离离交点非常近，会执行后续逻辑
                                {
                                    if (occ)
                                        a += grid_map_->getResolution();//如果被占据，增加步长，确保不会再尝试
                                    cps_.base_point[j].push_back((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j));
                                    cps_.direction[j].push_back((intersection_point - cps_.points.col(j)).normalized());
                                    break;
                                }
                ...
                //step 3
                if (got_intersection_id >= 0) //找到交点，进行路径调整，确保交点之后的控制点沿着之前的点相同的方向移动
                {
    ...
    {
        setBsplineInterval(ts);//控制b样条曲线的优化间隔
        bool flag_success = rebound_optimize();//执行优化
    ...
    {
        setControlPoints(init_points); //根据初始点。设置控制点
        setBsplineInterval(ts);

        bool flag_success = refine_optimize(); //在已有基础解的基础上进行局部精细化调整，比rebound更精细，可使用不同的优化方法
        optimal_points = cps_.points;
    ...
    bool BsplineOptimizer::rebound_optimize()
    {
        ...
        variable_num_ = 3 * (end_id - start_id);//3是因为存在xyz三个分量
        double final_cost;
        ...
        do{
            /* ---------- prepare ---------- */
            min_cost_ = std::numeric_limits<double>::max();//将最小成本初始化为一个极大数，之后优化更新这个值
            iter_num_=0;
            ...
            memcpy(q,cps_.points.data()+3*start_id,variable_num_ * sizeof(q[0]));//表示将cps_points中从start_id开始的连续部分复制到数组q
            lbfgs::lbfgs_parameter_t lbfgs_params;
            lbfgs::lbfgs_parameter_t lbfgs_params;
            lbfgs::lbfgs_load_default_parameters(&lbfgs_params);
            // 设置lbfgs优化参数，内存大小、最大迭代次数、梯度收敛阈值
                ...
                result == lbfgs::LBFGS_STOP)
            {
                ...
                // traj.evaluateDeBoorT(tmp) 表示b样条在t处的位置
            ...
            else
            {
                ...
                // while (ros::ok());
                 ...
                 (flag_force_return && force_stop_type_ == STOP_FOR_REBOUND && rebound_times <= 40));
        return success;
    }
