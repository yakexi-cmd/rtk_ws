...
namespace ego_planner
{
  ...
  {
    setUniformBspline(points, order, interval); //根据控制点，阶数，时间间隔，创建b样条曲线模型
  }
  ...
  {
    control_points_ = points;//控制点数
    p_ = order;//阶数
    interval_ = interval;//控制点间的距离

    n_ = points.cols() - 1;//控制点之间的区间数
    m_ = n_ + p_ + 1; //时间间隔数=控制点数+次数
    u_ = Eigen::VectorXd::Zero(m_ + 1); //时间节点数=间隔数+1
    for(int i=0;i<=m_;i++) //b样条初始化为均匀b样条，间隔相等
    {
      if(i<=p_)
      {
  ...
  Eigen::VectorXd UniformBspline::getKnot() { return this->u_; } //返回b样条的节点矢量
  ...
  */
  ...
  //给定位置，计算离他最近的时间点t_out和对应的最小距离dis_out
  void UniformBspline::getCloestT(const Eigen::Vector3d& pos, double& t_out, double& dis_out)
  {
      double total_time = getTimeSum();
      double dis = 1e10;
      double cloest_t = 0.0;
      double timestep = total_time / 100.0;
      for(double t = 0.0; t < total_time; t += timestep)
      {
          Eigen::Vector3d p = evaluateDeBoor(t);
          double d = (p - pos).norm();
          if(d < dis)
          {
              dis = d;
