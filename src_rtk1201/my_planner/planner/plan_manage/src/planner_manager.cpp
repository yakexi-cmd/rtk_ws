...
namespace ego_planner
{
                                          ...
                                          Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
    {
        ...
        /*** STEP 1: INIT ***/
        // start_pt - local_target_pt).norm()设置起点和目标点之间的距离
        double ts = (start_pt - local_target_pt).norm() > 0.1 ? pp_.ctrl_pt_dist / pp_.max_vel_ * 1.2 : pp_.ctrl_pt_dist / pp_.max_vel_ * 5; //距离近时显著增大时间间隔 pp_.ctrl_pt_dist / pp_.max_vel_ is too tense, and will surely exceed the acc/vel limits
        ...
        {
            point_set.clear();
            start_end_derivatives.clear();//存储起点和终点的导数
            ...
            if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
            {
                ...
                {
                    Eigen::Vector3d horizen_dir = ((start_pt - local_target_pt).cross(Eigen::Vector3d(0, 0, 1))).normalized();//计算和z轴的叉积，保证水平扰动始终垂直于起点和目标点的连线
                    Eigen::Vector3d vertical_dir = ((start_pt - local_target_pt).cross(horizen_dir)).normalized();//增加垂直方向的扰动
                ...
                do
                {
                    ts /= 1.5; //通过调整ts确保生成的轨迹点不稀疏，且数量足够
                    point_set.clear();
                    flag_too_far = false;
            ...
            else // 如果轨迹和之前的轨迹有关系，要处理如何基于先前的轨迹和目标点生成新的轨迹
            {
                std::cout << " enter  else \n";
                double t;
//                double t_cur = (ros::Time::now() - local_data_.start_time_).toSec();
                ...
                vector<double> pseudo_arc_length; //用来存储轨迹中各点的伪弧长，即路径上每个点之间的距离总和
                ...
                do
                {
                    ...
                    std::cout << "pseudo_arc_length.size() = " << pseudo_arc_length.size() << "\n";
                    try{
                        while((id <= pseudo_arc_length.size() - 2) && sample_length <= pseudo_arc_length.back())//确保索引不会超过范围，确保采样长度不会超过路径总长度
                        {
                ...
                if(point_set.size() > pp_.planning_horizen_ / pp.ctrl_pt_dist *3) //如果路径过长
                {
        ...
        t_start = ros::Time::now();
        /*** STEP 3:时间分配的重新调整 REFINE(RE-ALLOCATE TIME) IF NECESSARY ***/
        UniformBspline pos = UniformBspline(ctrl_pts, 3, ts);
        pos.setPhysicalLimits(pp_.max_vel_, pp_.max_acc_, pp_.feasibility_tolerance_);

        double ratio;
        bool flag_step_2_success = true;
        if (!pos.checkFeasibility(ratio, false)) //checkFeasibility判断是否符合物理约束（速度，加速度，平滑度等）
        {
        ...
        updateTrajInfo(pos, ros::Time::now(), (t_init + t_opt + t_refine).toSec());
    ...
    bool EGOPlannerManager::EmergencyStop(Eigen::Vector3d stop_pos)
    {
        ...
        updateTrajInfo(UniformBspline(control_points, 3, 1.0), ros::Time::now(), 1e5);
    ...
    void EGOPlannerManager::updateTrajInfo(const UniformBspline &position_traj, const ros::Time time_now, const double &planning_time)
    {
        ...
        local_data_.planning_time_ = planning_time;
    }
