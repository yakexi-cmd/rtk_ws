...
namespace ego_planner
{
    ...
    void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
    {
        ...
        //        }
            ...
            case GEN_NEW_TRAJ:
            {
                ...
                bool success = callReboundReplan(true, flag_random_poly_init); //生成新的轨迹
                if (success)
                {
                    auto info = &planner_manager_->local_data_;
                    info->start_time_ = ros::Time::now();
            ...
            //      if (planFromCurrentTraj())
                if (planFromCurrentTrajNotByT()) //表示重新规划成功，更新轨迹的起始时间并发布轨迹，同时发布停止命令已停止当前执行的动作
                {
                    auto info = &planner_manager_->local_data_;
                    info->start_time_ = ros::Time::now();
    ...
    void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e)
    {
        ...
        info->position_traj_.getCloestT(odom_pos_, t_cur1, dis_closest); //记录指定时刻t_cur1距离轨迹上的最近点，和该点与当前位置的距离
    ...
    void EGOReplanFSM::getLocalTarget() //根据全局轨迹生成局部目标点
    {
        double t;
