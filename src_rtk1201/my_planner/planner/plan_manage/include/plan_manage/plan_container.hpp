...
namespace ego_planner
{
    ...
    public:
        ...
        Eigen::Vector3d getPosition(double t)
        {
            ...
            else //如果t不再全局轨迹范围内，使用局部轨迹，local_traj_[0].evaluateDeBoor(tm + t - local_start_time_);评估局部轨迹的位置
            {
                double tm, tmp;
        ...
        int seg_num = floor(seg_length / dist_pt); //计算有多少个离散点
        ...
        //分别计算轨迹起始点和终止点的速度和加速度，并存储在start_end_derivative
